...
######################################## TRAIN_AWS 폴더 ###########################################
list = []
for filename in train_aws_files:
    df = pd.read_csv(filename, index_col=None, header=0,
                     encoding = 'utf-8-sig')
    
    list.append(df)
# print(list) # [35064 rows x 8 columns] 
# print(len(list))    # 30

train_aws_dataset = pd.concat(list, axis = 0,
                              ignore_index = True)

# print(train_aws_dataset)    # [1051920 rows x 8 columns]
...
for filename in test_input_files:
    ...
    list.append(df)     # 이어짐
# print(list)     # [7728 rows x 4 columns]]
# print(len(list))    # 17
test_aws_dataset = pd.concat(list, axis = 0, 
                          ignore_index= True)   # 새로운 index 생성
